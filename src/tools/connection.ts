...
import type { FioriConfig } from "../wdio/browser-factory.js";
...
export async function fioriConnect(args: FioriConnectArgs): Promise<Record<string, unknown>> {
  ...
  const headless = args.headless ?? process.env.FIORI_HEADLESS !== "false";
  ...
  const cfg: FioriConfig = { url, username, password, headless };

  try {
    await sessionManager.connect(cfg);
    return { connected: true, url, headless, state: sessionManager.getState() };
  } catch (err) {
    return {
      error: true,
